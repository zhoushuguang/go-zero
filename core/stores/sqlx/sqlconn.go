...
func NewSqlConn(driverName, datasource string, opts ...SqlOption) SqlConn {
	...
	conn.connProv = getConnProvider(conn)
...
func getConnProvider(sc *commonSqlConn) connProvider {
	return func(ctx context.Context) (*sql.DB, error) {
		replicaCount := len(sc.replicas)
		isReadOnly := isReadonly(ctx)

		if replicaCount == 0 || !isReadOnly {
			return getSqlConn(sc.driverName, sc.datasource)
		}
		...
		return getSqlConn(sc.driverName, dsn)
	}
}
